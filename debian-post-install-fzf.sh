...
fgdo_install_ouch() { # Installer ouch
	...
	sudo mkdir --parents /usr/local/share/zsh/site-functions
	sudo mv --force ./ouch-x86_64-unknown-linux-gnu/completions/_ouch /usr/local/share/zsh/site-functions/
	rm -rf ./ouch-*
	check $?
}
...
# https://github.com/mrizaln/madbfs
fgdo_install_madbfs() { # Installer madbfs
	install_name madbfs || return
	dra download --select madbfs.tar.gz mrizaln/madbfs
	atool --extract madbfs.tar.gz
	sudo mv --force ./madbfs/* /usr/local/bin/
	sudo chmod +x /usr/local/bin/madbfs*
	rm --force ./madbfs.tar.gz
	rm --force --recursive ./madbfs
	check $?
}
...
fgdo_make_symlinks() { # Créer des liens symboliques
	...
	sudo ln -s /usr/share/man/man1/batcat.1.gz /usr/local/share/man/man1/bat.1.gz
	sudo ln -s /usr/share/man/man1/fdfind.1.gz /usr/local/share/man/man1/fd.1.gz
	check $?
}
...
- puis gsettings set org.gnome.desktop.interface gtk-theme 'Mint-Y-Purple'
- gsettings set org.gnome.desktop.interface icon-theme 'Mint-Y-Purple'


- Copier la musique, les photos et vidéos.

- Éventuellement, installer grub-customizer, notamment pour modifier facilement la taille de la police (32 est bien!).
